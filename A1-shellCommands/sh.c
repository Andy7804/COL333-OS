...
int
my_strncmp(const char *p, const char *q, uint n)
{
  while(n > 0 && *p && *p == *q) {
    n--;
    p++;
...
main(void)
{
  ...
  while(getcmd(buf, sizeof(buf)) >= 0){
    ...
    if(my_strncmp(buf, "chmod", 5) == 0){
      ...
      // Copy the file name until the next space or newline.
      int i = 0;
      while(*p && *p != ' ' && *p != '\n' && i < sizeof(file)-1){
        file[i++] = *p;
        p++;
      ...
      // Skip spaces to get to the mode argument.
      while(*p == ' ')
        p++;
      
      // Check if a mode argument exists
      if(*p == '\0'){
        printf(2, "usage: chmod <filename> <mode>\n");
        continue;
      }
      ...
      // Call the user-level chmod system call.
