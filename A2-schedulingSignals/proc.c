...
void handle_signal(int signum) {
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if(p->state != UNUSED && p->pid > 2) {
      ...
      else if(signum == SIGCUSTOM) {
        p->sigcustom_pending = 1;
      }
    }
  }
  release(&ptable.lock);
...
custom_fork(int start_later_flag, int exec_time)
{
  ...
  // Allocate process, same as normal fork
  if((np = allocproc()) == 0){
    return -1;
  }

  // Copy process state from parent, same as normal fork
  if((np->pgdir = copyuvm(curproc->pgdir, curproc->sz)) == 0){
    kfree(np->kstack);
    np->kstack = 0;
    np->state = UNUSED;
  ...
  *np->tf = *curproc->tf;
  ...
  // Clear %eax so that fork returns 0 in the child, same as normal fork
  np->tf->eax = 0;
  ...
  // Make process runnable if start_later_flag is false
  acquire(&ptable.lock);
...
exit(void)
{
  ...
  int tat = curproc->exit_time - curproc->creation_time;
  int rt = curproc->first_scheduled - curproc->creation_time;
  int wt = curproc->total_waiting_time;
...
scheduler(void)
{
  ...
  for(;;){
    ...
    if(highest_priority != 0){
      ...
      if(p->exec_time > 0 && p->cpu_ticks_used >= p->exec_time){
        // Kill the process
        p->killed = 1;
